	...
	addSensorsFromNode(stNode) {
	  ...
	  // · · · · · ·  # #  · · · · · ·  ###  · · · · · ·  # # · · · · · · |/\|···  
		...
		stNode.socket.on(smngr.CONSTANTS.Messages.SensorsList, function(data){
			...
			smngr._msg_SensorsList(data, stNode, {"data" : data});
			  
		  });
		
		
		// Map message SensorOptions
		stNode.socket.on(smngr.CONSTANTS.Messages.SensorOptions, function(data) {

			smngr._msg_SensorOptions(data, stNode, {
...
//		let _nodeID = nodeID;
	...
	getOptionsOfSensor(sns) {
		...
		controlSocket.emit(smngr.CONSTANTS.Messages.getSensorOptions, 
				{"sensorID" : sns.config.id});	// Emit message getSensorOptions
	...
	setOptionsOfSensor(sns, options) {
		...
		controlSocket.emit(smngr.CONSTANTS.Messages.setSensorOptions, 
				{"sensorID" : sns.config.id, "options" : options});	// Emit message setSensorOptions
	...
	_msg_SensorOptions(msg, stNode, options) {
		...
		console.log('<*> ST NodesNetManager._msg_SensorOptions');	// TODO REMOVE DEBUG LOG
		console.log(options);	// TODO REMOVE DEBUG LOG
