...
class NodesNetService {
	...
	_msg_DataChannelCreated(msg, socket, options){
		...
		if (dcSearch.dataChannel.config._netState == undefined || 
				dcSearch.dataChannel.config._netState != nnetm.CONSTANTS.States.DCstate_Config) {
			throw "Bad Data channel state.";
		}
		
		dcSearch.dataChannel.config._netState = nnetm.CONSTANTS.States.DCstate_Ready;
	...
	_deleteDConServer(dch, node) {
		...
		console.log('<*> ST NodesNetService._deleteDConServer');	// TODO REMOVE DEBUG LOG
		...
		try {
			...
			// TODO: handle exception
			console.log('<EEE> ST NodesNetService._deleteDConServer');	// TODO REMOVE DEBUG LOG
			console.log(e.message);	// TODO REMOVE DEBUG LOG
		}
	...
	_synchroNodeChannels(node, dchnlistOfNode, fromNode) {
		...
		var dcSearch = nnets.nodesNetManager.getDataChannelsOfNode( node.config.nodeID );	// Search data channels of node
